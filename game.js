/* webapp/game.js (v30.0 - Pixel Perfect Mobile Fix) */
...
window.onload = function() {
    ...
    document.querySelectorAll('button').forEach(btn => {
        btn.addEventListener('click', () => { if(!btn.disabled) tg.HapticFeedback.impactOccurred('light'); });
    });
};
...
function initChart() {
    ...
    chart = LightweightCharts.createChart(container, {
        ...
        height: container.clientHeight, // حالا ارتفاع دقیق را از CSS می‌گیرد
        layout: {
        ...
        rightPriceScale: {
            ...
            visible: true,
            scaleMargins: { top: 0.1, bottom: 0.05 }, // حاشیه کم برای موبایل
        },
        timeScale: {
            borderColor: 'transparent',
            timeVisible: true,
    ...
    // Resize Observer برای فیت شدن دقیق
...
function updateGameUI(data) {
    ...
    if (data.round) { updateTimerCircle(data.round.time_left); updateRoundStatus(data); }
    if (data.history) { updateHistoryRibbon(data.history); checkWinLoss(data.history); }
}
...
function updateRoundStatus(data) {
    ...
    } else if (isLocked) { elStatus.innerHTML = `<span style="color:#F6465D">LOCKED 🔒</span>`; }
    else { elStatus.innerHTML = `<span style="color:#0ECB81">OPEN 🟢</span>`; }
...
function checkWinLoss(history) {
    ...
    if (round) {
        localStorage.removeItem('last_bet_round_id'); localStorage.removeItem('last_bet_prediction');
        if (round.result === myPrediction) { SoundFX.win(); tg.HapticFeedback.notificationOccurred('success'); confetti({ particleCount: 150, spread: 80, origin: { y: 0.7 } }); showToast(`🎉 WIN! Settlement Complete.`); }
        else { SoundFX.lose(); tg.HapticFeedback.notificationOccurred('error'); showToast(`❌ Position Closed.`); }
    }
}
