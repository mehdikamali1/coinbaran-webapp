/* webapp/script.js (v97.0 - UPGRADE: XP PROGRESSION Harmonized) */
(function () {
    'use strict';
...
        // UPGRADE: Use the server's user_levels_config for accurate progression
        const rawXpString = (data.xp_balance || "0").replace(/,/g, '');
        const currentXp = parseInt(rawXpString) || 0;
        if (data.user_levels_config) {
            updateLevelProgress(currentXp, data.user_levels_config);
        } else {
            // Fallback if server config is missing (unlikely after fix)
...
    // UPGRADED FUNCTION: Uses the configuration data sent by the server
    function updateLevelProgress(xp, levelsConfig) {
        if (!els.xpFill || !els.levelBadge || !levelsConfig) return;

        const levelKeys = Object.keys(levelsConfig);
        if (levelKeys.length === 0) { 
            // Fallback in case config is empty
            els.levelBadge.innerText = 'LVL 1'; 
            els.xpFill.style.width = '0%';
            els.nextLevelText.innerText = 'XP system disabled';
            return;
        }
        
        // Sort levels by XP required to ensure correct progression check
        const sortedLevels = levelKeys
            .map(key => ({ 
                name: key.charAt(0).toUpperCase() + key.slice(1), 
                xp: levelsConfig[key].xp_to_reach 
            }))
            .sort((a, b) => a.xp - b.xp);

        let currentLevelName = sortedLevels[0].name;
        let prevThreshold = 0;
...
                prevThreshold = level.xp;
                
                // Check for next level
                if (i + 1 < sortedLevels.length) {
                    nextThreshold = sortedLevels[i + 1].xp;
                    nextLevelName = sortedLevels[i + 1].name;
                } else {
                    // Max level reached
...
                }
            } else {
                // User is between prevThreshold and current level.
                nextThreshold = level.xp;
...
        els.levelBadge.innerText = currentLevelName;
        
        if (els.nextLevelText) {
            if (nextLevelName === 'MAX') {
                 els.nextLevelText.innerText = `Maximum Level Reached`;
            } else {
                 const remaining = nextThreshold - xp;
                 els.nextLevelText.innerText = `${remaining.toLocaleString()} XP to ${nextLevelName}`;
            }
        }
    }
    
